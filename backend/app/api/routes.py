...
from app.schemas.requests import ChatMessageInput, ConfirmationInput, DecisionResponse, RequestInput, ReviewInput
...
def _has_maintenance_detail(text: str) -> bool:
    ...
    return any(parsed.get(key, "Not specified") != "Not specified" for key in ("issue", "location", "floor"))
...
def _handle_maintenance(user_key: str, text_en: str, role, lang: str) -> dict:
    ...
    request, policy, permitted, audit_id = request_service.create(detail_text, role, "MAINTENANCE", dict(merged), user_id=user_key)
...
def _handle_grievance(user_key: str, text_en: str, role, lang: str) -> dict:
    """Log a non-facility complaint as a grievance and escalate it to a human officer."""
    request, policy, permitted, audit_id = request_service.create(text_en, role, "GRIEVANCE", {"summary": text_en.strip()[:200]}, user_id=user_key)
    CONVERSATION.pop(user_key, None)
    response = serialize(request, policy, permitted, audit_id)
    response.message = translator.localize(response.message, lang)
    return {"type": "decision", "decision": response.model_dump(), "language": lang}
...
def serialize(request, policy=None, permitted=True, audit_id=""):
    ...
    return DecisionResponse(request_id=request.id, intent=request.intent, entities=request.entities, decision=request.decision.value, status=request.status.value, policy={"id": request.policy_id, "name": request.policy_name, "confidence": policy.confidence if policy else .94, "source_section": policy.source_section if policy else "", "citation": policy.citation if policy else "", "uncertain": policy.uncertain if policy else False}, permission="Allowed" if permitted else "Denied", risk=request.risk, evidence="Verified" if request.policy_id else "Insufficient", message=request.reason, audit_id=audit_id, requires_confirmation=request.status.value == "AWAITING_CONFIRMATION" and request.decision.value == "ASK" and not needs_details)
...
def assistant(payload: RequestInput, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
        ...
        if user_key not in CONVERSATION:
            ...
            for saved in reversed(prior[:-1]):  # frontend stores the current user turn first
                if saved.role == "assistant":
                    try:
                        action = (json.loads(saved.payload) or {}).get("action", {})
    ...
    if flow == "certificate":
        ...
        fresh_intent, _, _, _ = propose(text_en)
        ...
        if flow == "certificate" and (_is_question(text_en) or _wants_details(text_en)):
            ...
            if new_policy and is_grounded(new_policy):
                ...
                if answer:
                    return {"type": "message", "message": translator.localize(answer, lang), "language": lang}
        if flow == "certificate":
            LAST_POLICY[user_key] = "ACA-BONAFIDE-011"
    ...
    if flow == "offer_complaint":
        ...
        if _has_maintenance_detail(text_en):
            return _handle_maintenance(user_key, text_en, payload.role, lang)
    ...
    if flow == "complaint_intake":
        ...
        if _has_maintenance_detail(text_en):
            return _handle_maintenance(user_key, text_en, payload.role, lang)
        if len(text_en.split()) >= 3:
            return _handle_grievance(user_key, text_en, payload.role, lang)
        return {"type": "message", "message": translator.localize("Could you describe the issue in a bit more detail \u2014 what happened, and where?", lang), "language": lang}
    ...
    if flow == "maintenance":
        ...
        switch_intent, _, _, _ = propose(text_en)
    ...
    if not stage and not flow and _wants_to_complain(text_en) and not _has_maintenance_detail(text_en):
        CONVERSATION[user_key] = {"flow": "complaint_intake"}
        return {"type": "message", "message": translator.localize("Sure \u2014 I can help you file a complaint. What would you like to report? Please describe the issue, and if it's about a facility, tell me the block/room and floor.", lang), "language": lang}
    ...
    if stage in ("collecting", "confirming"):
        if has_detail:
            ...
            intent, _, _, _ = propose(text_en)
            if intent in ("MAINTENANCE", "CERTIFICATE"):
    ...
    if intent == "UNSUPPORTED":
        ...
        if top and top.answer and is_grounded(top):
            ...
            if top.policy_id == "GRV-ESCAL-004":
                CONVERSATION[user_key] = {"flow": "offer_complaint"}
            return {"type": "message", "message": translator.localize(top.answer, lang), "language": lang}
        ...
        if not _is_campus_related(text_en):
            message = "That request is outside CampusFlow's scope. I can help with institutional campus services and verified campus policies, but I can't book flights or other external services."
            return {"type": "message", "message": translator.localize(message, lang), "language": lang}
...
def requests(user: User = Depends(get_current_user)):
    ...
    if user.role not in ("ADMIN", "APPROVER"):
        rows = [r for r in rows if r.user_id == user.email]
    return [{"id": r.id, "type": r.intent.replace("_", " ").title(), "status": r.status.value, "decision": r.decision.value, "created_at": r.created_at.isoformat(), "entities": r.entities} for r in rows]
